...
class Registration2D3DDataset(Dataset):
    ...
    def __init__(self, data_path, phase=None, transform=None, option=None):
        ...
        """
        if phase == "debug":
            self.data_id_path = data_path + "/train/data_id.npy"
        else:
    ...
    def _read_case(self, case_id_list, img_label_dic):
        ...
        for case_id in case_id_list:
            ...
            if self.apply_hu_clip:
                source_img = self._normalize_intensity(source_img, linear_clip=True, clip_range=[-1000, 0])
            else:
                source_img = self._normalize_intensity(source_img, linear_clip=True)
            if self.has_label:
            ...
            if self.apply_hu_clip:
                target_img = self._normalize_intensity(target_img, linear_clip=True, clip_range=[-1000, 0])
            else:
                target_img = self._normalize_intensity(target_img, linear_clip=True)
            if self.has_label:
