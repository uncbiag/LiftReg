...
class ImageViewer3D_Sliced(ImageViewer3D):
    ...
    def show(self):
        ...
        plt.gcf().colorbar(cim, cax=cax, orientation='vertical').ax.tick_params(labelsize=2)
        self.display_title()

class ImageViewer3D_Sliced_optical_flow(ImageViewer3D_Sliced):
    """
    3D image viewer specialization to 3D sliced viewing
    """
    ...
    def __init__(self, ax, data, sliceDim, textStr='Slice', showColorbar=False,using_grid=False):
        ...
        :param showColorbar: (bool) should a colorbar be displayed? 
        """
        self.sliceDim = 1 + sliceDim
        super(ImageViewer3D_Sliced_optical_flow,self).__init__(ax,data)
    
    def _get_quiver_data(self, img, sample_step=4):
        W, H = img.shape[1:]
        x, y = np.linspace(0, W-1, int(W/sample_step)), np.linspace(0, H-1, int(H/sample_step))
        xy, yx = np.meshgrid(x,y)
        idx = [slice(1, 3), slice(0,3,2), slice(0,2)]
        
        return [xy, yx], img[idx[self.flow_sliceDim], 0::sample_step, 0::sample_step]
    ...
    def show(self):
        ...
        plt.cla()
        #print('debugging {}'.format(self.index), 'slice_dim{}'.format(self.sliceDim),'img_shape{}'.format(self.data.shape))
        flow_at_index = self._get_slice_at_dimension(self.index)
        quiver_xy, quiver_uv = self._get_quiver_data(flow_at_index, sample_step=8)
        self.ax.quiver(quiver_xy[1], quiver_xy[0], np.zeros_like(quiver_uv[1]), quiver_uv[0], angles='xy')
        self.ax.yaxis.grid(self.using_grid)
        self.ax.xaxis.grid(self.using_grid)
        if(self.using_grid):
            self.ax.xaxis.set_tick_params(labelsize=4)
...
class ImageViewer3D_Sliced_Contour(ImageViewer3D_Sliced):
    ...
    def show_contours(self):
        ...
        for d in range(0,self.sliceDim):
            plt.contour(phiSliced[d,:,:], np.linspace(-1,1,70), cmap='Dark2', linestyles='solid', linewidths=0.08)
    ...
    def show(self):
        ...
        plt.cla()
        #print('debugging {}'.format(self.index), 'slice_dim{}'.format(self.sliceDim),'img_shape{}'.format(self.data.shape))
        cim = self.ax.imshow(self._get_slice_at_dimension(self.index), cmap='gray', vmin='-1', vmax= '1')
        self.ax.yaxis.grid(self.using_grid)
        self.ax.xaxis.grid(self.using_grid)
        if(self.using_grid):
            self.ax.xaxis.set_tick_params(labelsize=4)
...
def _create_some_test_data(dim=2):
    if dim == 2:
        ...
        a = np.linspace(0,50)
        b = np.ones((80))
        x2d, y2d = np.meshgrid(a, b)
        data = np.ones((3, 50, 80, 50))
        data[0] *= 4
        data[1] *= -4
        data[2] = 0
    return data

def test_viewer(viewer_name):
    """
    simple test viewer
    """
    if viewer_name == "ImageViewer3D_Sliced":
        data = _create_some_test_data(dim=2)
        ...
        plt.show()
    elif viewer_name == "ImageViewer3D_optical_flow_Sliced":
        data = _create_some_test_data(dim=3)
        fig,ax = plt.subplots(1,3)

        # plt.setp(plt.gcf(), 'facecolor', 'white')

        ivx = ImageViewer3D_Sliced_optical_flow( ax[0], data, 0, 'X slice')
        ivy = ImageViewer3D_Sliced_optical_flow( ax[1], data, 1, 'Y slice')
        ivz = ImageViewer3D_Sliced_optical_flow( ax[2], data, 2, 'Z slice')
        ...
        feh.synchronize([ax[0], ax[1], ax[2]])
        plt.axis("on")
        plt.savefig("./temp/debug.png", dpi=300)

if __name__ == '__main__':
    test_viewer("ImageViewer3D_optical_flow_Sliced")
